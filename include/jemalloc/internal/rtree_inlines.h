...
rtree_elm_t *rtree_subtree_tryread(rtree_t *rtree, unsigned level,
                                   bool dependent);
rtree_elm_t *rtree_subtree_read(tsdn_t *tsdn, rtree_t *rtree, unsigned level,
                                bool dependent);
...
JEMALLOC_ALWAYS_INLINE unsigned rtree_start_level(const rtree_t *rtree,
                                                  uintptr_t key) {
    ...
    start_level =
        rtree->start_level[(lg_floor(key) + 1) >> LG_RTREE_BITS_PER_LEVEL];
...
rtree_ctx_start_level(const rtree_t *rtree, const rtree_ctx_t *rtree_ctx,
     ...
     */
    start_level =
        rtree->start_level[(lg_floor(key_diff) + 1) >> LG_RTREE_BITS_PER_LEVEL];
    assert(start_level < rtree->height);
    return (start_level);
}

JEMALLOC_ALWAYS_INLINE uintptr_t rtree_subkey(rtree_t *rtree, uintptr_t key,
                                              unsigned level) {
    return ((key >>
             ((ZU(1) << (LG_SIZEOF_PTR + 3)) - rtree->levels[level].cumbits)) &
            ((ZU(1) << rtree->levels[level].bits) - 1));
}

JEMALLOC_ALWAYS_INLINE bool rtree_node_valid(rtree_elm_t *node) {
    return ((uintptr_t)node != (uintptr_t)0);
}

JEMALLOC_ALWAYS_INLINE rtree_elm_t *rtree_child_tryread(rtree_elm_t *elm,
                                                        bool dependent) {
    rtree_elm_t *child;
...
JEMALLOC_ALWAYS_INLINE extent_t *rtree_elm_read(rtree_elm_t *elm,
                                                bool dependent) {
    extent_t *extent;
...
JEMALLOC_INLINE void rtree_elm_write(rtree_elm_t *elm, const extent_t *extent) {
...
rtree_subtree_tryread(rtree_t *rtree, unsigned level, bool dependent) {
    ...
    if (!dependent && unlikely(!rtree_node_valid(subtree))) {
        subtree =
            (rtree_elm_t *)atomic_read_p(&rtree->levels[level].subtree_pun);
    }
    assert(!dependent || subtree != NULL);
    return (subtree);
}

JEMALLOC_ALWAYS_INLINE rtree_elm_t *rtree_subtree_read(tsdn_t *tsdn,
                                                       rtree_t *rtree,
                                                       unsigned level,
                                                       bool dependent) {
    rtree_elm_t *subtree;
    ...
    if (dependent || init_missing) {
        if (likely(rtree_ctx->valid)) {
            if (key == rtree_ctx->key)
                ...
                unsigned no_ctx_start_level = rtree_start_level(rtree, key);
                unsigned ctx_start_level;

                if (likely(no_ctx_start_level <= rtree_ctx->start_level &&
                           (ctx_start_level =
                                rtree_ctx_start_level(rtree, rtree_ctx, key)) >=
                    ...
                    node = init_missing
                               ? rtree_subtree_read(
                                     tsdn, rtree, no_ctx_start_level, dependent)
                               : rtree_subtree_tryread(
                                     rtree, no_ctx_start_level, dependent);
            ...
            start_level = no_ctx_start_level;
            node = init_missing
                       ? rtree_subtree_read(tsdn, rtree, no_ctx_start_level,
                                            dependent)
        ...
        node = init_missing
                   ? rtree_subtree_read(tsdn, rtree, start_level, dependent)
...
#define RTREE_GET_SUBTREE(level)                                               \
    case level:                                                                \
        ...
        subkey = rtree_subkey(rtree, key, level - RTREE_GET_BIAS);             \
        node = init_missing                                                    \
                   ? rtree_child_read(tsdn, rtree, &node[subkey],              \
                                      level - RTREE_GET_BIAS, dependent)       \
                   : rtree_child_tryread(&node[subkey], dependent);            \
        if (dependent || init_missing) {                                       \
            rtree_ctx->elms[level - RTREE_GET_BIAS + 1] = node;                \
        }                                                                      \
/* Fall through. */
...
JEMALLOC_INLINE bool rtree_write(tsdn_t *tsdn, rtree_t *rtree,
                                 rtree_ctx_t *rtree_ctx, uintptr_t key,
...
JEMALLOC_ALWAYS_INLINE extent_t *rtree_read(tsdn_t *tsdn, rtree_t *rtree,
...
JEMALLOC_INLINE rtree_elm_t *rtree_elm_acquire(tsdn_t *tsdn, rtree_t *rtree,
                                               rtree_ctx_t *rtree_ctx,
                                               uintptr_t key, bool dependent,
                                               bool init_missing) {
    rtree_elm_t *elm;
    ...
    elm =
        rtree_elm_lookup(tsdn, rtree, rtree_ctx, key, dependent, init_missing);
...
JEMALLOC_INLINE void rtree_elm_write_acquired(tsdn_t *tsdn,
                                              const rtree_t *rtree,
                                              rtree_elm_t *elm,
                                              const extent_t *extent) {
    assert(((uintptr_t)extent & (uintptr_t)0x1) == (uintptr_t)0x0);
...
JEMALLOC_INLINE void rtree_elm_release(tsdn_t *tsdn, const rtree_t *rtree,
                                       rtree_elm_t *elm) {
...
JEMALLOC_INLINE void rtree_clear(tsdn_t *tsdn, rtree_t *rtree,
                                 rtree_ctx_t *rtree_ctx, uintptr_t key) {
    rtree_elm_t *elm;
