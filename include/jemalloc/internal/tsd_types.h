...
#define malloc_tsd_protos(a_attr, a_name, a_type)                              \
    a_attr bool a_name##tsd_boot0(void);                                       \
    a_attr void a_name##tsd_boot1(void);                                       \
    a_attr bool a_name##tsd_boot(void);                                        \
    a_attr bool a_name##tsd_booted_get(void);                                  \
    a_attr a_type *a_name##tsd_get(bool init);                                 \
    a_attr void a_name##tsd_set(a_type *val);
...
#define malloc_tsd_data(a_attr, a_name, a_type, a_initializer)                 \
    a_attr __thread a_type JEMALLOC_TLS_MODEL a_name##tsd_tls = a_initializer; \
    a_attr __thread bool JEMALLOC_TLS_MODEL a_name##tsd_initialized = false;   \
...
#define malloc_tsd_data(a_attr, a_name, a_type, a_initializer)                 \
    ...
    a_attr a_name##tsd_wrapper_t a_name##tsd_boot_wrapper = {false,            \
                                                             a_initializer};   \
    a_attr bool a_name##tsd_booted = false;
#else
#define malloc_tsd_data(a_attr, a_name, a_type, a_initializer)                 \
...
/* malloc_tsd_funcs(). */
#ifdef JEMALLOC_MALLOC_THREAD_CLEANUP
#define malloc_tsd_funcs(a_attr, a_name, a_type, a_initializer, a_cleanup)     \
    /* Initialization/cleanup. */                                              \
    a_attr bool a_name##tsd_cleanup_wrapper(void) {                            \
    ...
    a_attr bool a_name##tsd_boot0(void) {                                      \
        if (a_cleanup != malloc_tsd_no_cleanup) {                              \
            malloc_tsd_cleanup_register(&a_name##tsd_cleanup_wrapper);         \
        }                                                                      \
    ...
    a_attr bool a_name##tsd_get_allocates(void) {                              \
        return (false);                                                        \
    }                                                                          \
    /* Get/set. */                                                             \
    a_attr a_type *a_name##tsd_get(bool init) {                                \
...
#define malloc_tsd_funcs(a_attr, a_name, a_type, a_initializer, a_cleanup)     \
    ...
    a_attr bool a_name##tsd_boot0(void) {                                      \
        if (a_cleanup != malloc_tsd_no_cleanup) {                              \
            if (pthread_key_create(&a_name##tsd_tsd, a_cleanup) != 0)          \
                return (true);                                                 \
        }                                                                      \
        a_name##tsd_booted = true;                                             \
        return (false);                                                        \
...
#define malloc_tsd_funcs(a_attr, a_name, a_type, a_initializer, a_cleanup)     \
    ...
    a_attr bool a_name##tsd_cleanup_wrapper(void) {                            \
        ...
        a_name##tsd_wrapper_t *wrapper =                                       \
            (a_name##tsd_wrapper_t *)TlsGetValue(a_name##tsd_tsd);             \
        SetLastError(error);                                                   \
                                                                               \
        if (wrapper == NULL)                                                   \
            return (false);                                                    \
        if (a_cleanup != malloc_tsd_no_cleanup && wrapper->initialized) {      \
    ...
    a_attr void a_name##tsd_wrapper_set(a_name##tsd_wrapper_t *wrapper) {      \
    ...
    a_attr a_name##tsd_wrapper_t *a_name##tsd_wrapper_get(bool init) {         \
        DWORD error = GetLastError();                                          \
        a_name##tsd_wrapper_t *wrapper =                                       \
            (a_name##tsd_wrapper_t *)TlsGetValue(a_name##tsd_tsd);             \
        SetLastError(error);                                                   \
        ...
        if (init && unlikely(wrapper == NULL)) {                               \
            wrapper = (a_name##tsd_wrapper_t *)malloc_tsd_malloc(              \
...
#define malloc_tsd_funcs(a_attr, a_name, a_type, a_initializer, a_cleanup)     \
    /* Initialization/cleanup. */                                              \
    a_attr void a_name##tsd_cleanup_wrapper(void *arg) {                       \
        ...
        if (a_cleanup != malloc_tsd_no_cleanup && wrapper->initialized) {      \
            ...
            if (wrapper->initialized) {                                        \
                /* Trigger another cleanup round. */                           \
                if (pthread_setspecific(a_name##tsd_tsd, (void *)wrapper)) {   \
                    malloc_write("<jemalloc>: Error"                           \
                                 " setting TSD for " #a_name "\n");            \
                    if (opt_abort)                                             \
                        abort();                                               \
    ...
    a_attr a_name##tsd_wrapper_t *a_name##tsd_wrapper_get(bool init) {         \
        a_name##tsd_wrapper_t *wrapper =                                       \
            (a_name##tsd_wrapper_t *)pthread_getspecific(a_name##tsd_tsd);     \
                                                                               \
        if (init && unlikely(wrapper == NULL)) {                               \
            tsd_init_block_t block;                                            \
            wrapper = (a_name##tsd_wrapper_t *)tsd_init_check_recursion(       \
