...
#define SC(index, lg_grp, lg_delta, ndelta, psz, bin, pgs, lg_delta_lookup)    \
    BIN_INFO_bin_##bin(                                                        \
        (1U << lg_grp) + (ndelta << lg_delta), (pgs << LG_PAGE),               \
...
static size_t arena_extent_dirty_npages(const extent_t *extent) {
...
extent_t *arena_extent_cache_alloc(tsdn_t *tsdn, arena_t *arena,
    ...
    extent = arena_extent_cache_alloc_locked(
        tsdn, arena, r_extent_hooks, new_addr, size, 0, alignment, zero, false);
    malloc_mutex_unlock(tsdn, &arena->lock);

    return (extent);
}

static void arena_extent_cache_dalloc_locked(tsdn_t *tsdn, arena_t *arena,
                                             extent_hooks_t **r_extent_hooks,
                                             extent_t *extent) {
    malloc_mutex_assert_owner(tsdn, &arena->lock);
...
JEMALLOC_INLINE_C void *arena_slab_reg_alloc(tsdn_t *tsdn, extent_t *slab,
...
size_t arena_slab_regind(extent_t *slab, szind_t binind, const void *ptr) {
    ...
    assert(((uintptr_t)ptr - (uintptr_t)extent_addr_get(slab)) %
               (uintptr_t)arena_bin_info[binind].reg_size ==
           0);
...
JEMALLOC_INLINE_C void arena_slab_reg_dalloc(tsdn_t *tsdn, extent_t *slab,
...
static void arena_large_malloc_stats_update(arena_t *arena, size_t usize) {
...
static void arena_large_malloc_stats_update_undo(arena_t *arena, size_t usize) {
    szind_t index, hindex;
...
static void arena_large_dalloc_stats_update(arena_t *arena, size_t usize) {
    szind_t index, hindex;
...
static void arena_large_reset_stats_cancel(arena_t *arena, size_t usize) {
...
static void arena_large_ralloc_stats_update(arena_t *arena, size_t oldusize,
                                            size_t usize) {
    arena_large_dalloc_stats_update(arena, oldusize);
    arena_large_malloc_stats_update(arena, usize);
}

static extent_t *arena_extent_alloc_large_hard(tsdn_t *tsdn, arena_t *arena,
                                               extent_hooks_t **r_extent_hooks,
                                               size_t usize, size_t alignment,
                                               bool *zero) {
    extent_t *extent;
    bool commit = true;
...
extent_t *arena_extent_alloc_large(tsdn_t *tsdn, arena_t *arena, size_t usize,
    ...
    extent =
        arena_extent_cache_alloc_locked(tsdn, arena, &extent_hooks, NULL, usize,
                                        large_pad, alignment, zero, false);
    malloc_mutex_unlock(tsdn, &arena->lock);
    if (extent == NULL) {
        extent = arena_extent_alloc_large_hard(tsdn, arena, &extent_hooks,
...
void arena_extent_dalloc_large(tsdn_t *tsdn, arena_t *arena, extent_t *extent,
                               bool locked) {
    extent_hooks_t *extent_hooks = EXTENT_HOOKS_INITIALIZER;
    ...
    if (config_stats) {
        arena_large_dalloc_stats_update(arena, extent_usize_get(extent));
...
static void arena_decay_deadline_init(arena_t *arena) {
    /*
...
static bool arena_decay_deadline_reached(const arena_t *arena,
                                         const nstime_t *time) {
    return (nstime_compare(&arena->decay.deadline, time) <= 0);
}

static size_t arena_decay_backlog_npages_limit(const arena_t *arena) {
...
static void arena_decay_backlog_update_last(arena_t *arena) {
    size_t ndirty_delta = (arena->ndirty > arena->decay.nunpurged)
                              ? arena->ndirty - arena->decay.nunpurged
                              : 0;
    arena->decay.backlog[SMOOTHSTEP_NSTEPS - 1] = ndirty_delta;
}

static void arena_decay_backlog_update(arena_t *arena, uint64_t nadvance_u64) {
    if (nadvance_u64 >= SMOOTHSTEP_NSTEPS) {
        memset(arena->decay.backlog, 0,
        ...
        if (nadvance_z > 1) {
            memset(&arena->decay.backlog[SMOOTHSTEP_NSTEPS - nadvance_z], 0,
...
static void arena_decay_epoch_advance_helper(arena_t *arena,
                                             const nstime_t *time) {
...
static void arena_decay_epoch_advance_purge(tsdn_t *tsdn, arena_t *arena) {
...
static void arena_decay_init(arena_t *arena, ssize_t decay_time) {
...
static bool arena_decay_time_valid(ssize_t decay_time) {
    if (decay_time < -1) {
        return (false);
    }
...
static void arena_maybe_purge_helper(tsdn_t *tsdn, arena_t *arena) {
    nstime_t time;
    ...
    if (unlikely(!nstime_monotonic() &&
                 nstime_compare(&arena->decay.epoch, &time) > 0)) {
        /*
...
static size_t arena_dirty_count(tsdn_t *tsdn, arena_t *arena) {
    extent_t *extent;
    size_t ndirty = 0;

    malloc_mutex_lock(tsdn, &arena->extents_mtx);

    for (extent = qr_next(&arena->extents_dirty, qr_link);
         extent != &arena->extents_dirty; extent = qr_next(extent, qr_link)) {
...
static size_t arena_stash_dirty(tsdn_t *tsdn, arena_t *arena,
                                extent_hooks_t **r_extent_hooks,
                                size_t ndirty_limit,
    ...
    for (extent = qr_next(&arena->extents_dirty, qr_link);
         extent != &arena->extents_dirty; extent = next) {
        size_t npages;
        bool zero, commit;
        ...
        zero = false;
        commit = false;
        textent = extent_alloc_cache_locked(
            tsdn, arena, r_extent_hooks, extent_base_get(extent),
            extent_size_get(extent), 0, PAGE, &zero, &commit, false);
...
static size_t arena_purge_stashed(tsdn_t *tsdn, arena_t *arena,
                                  extent_hooks_t **r_extent_hooks,
                                  extent_t *purge_extents_sentinel) {
    ...
    for (extent = qr_next(purge_extents_sentinel, qr_link);
         extent != purge_extents_sentinel; extent = next) {
        if (config_stats) {
            nmadvise++;
        }
...
static void arena_bin_slabs_nonfull_insert(arena_bin_t *bin, extent_t *slab) {
    assert(extent_slab_data_get(slab)->nfree > 0);
    extent_heap_insert(&bin->slabs_nonfull, slab);
}

static void arena_bin_slabs_nonfull_remove(arena_bin_t *bin, extent_t *slab) {
    extent_heap_remove(&bin->slabs_nonfull, slab);
}

static extent_t *arena_bin_slabs_nonfull_tryget(arena_bin_t *bin) {
...
static void arena_bin_slabs_full_insert(arena_bin_t *bin, extent_t *slab) {
    assert(extent_slab_data_get(slab)->nfree == 0);
    extent_ring_insert(&bin->slabs_full, slab);
}

static void arena_bin_slabs_full_remove(extent_t *slab) {
...
void arena_reset(tsd_t *tsd, arena_t *arena) {
    ...
    for (extent = ql_last(&arena->large, ql_link); extent != NULL;
         extent = ql_last(&arena->large, ql_link)) {
    ...
    for (i = 0; i < NBINS; i++) {
        ...
        for (slab = qr_next(&bin->slabs_full, qr_link);
             slab != &bin->slabs_full;
             slab = qr_next(&bin->slabs_full, qr_link)) {
...
static extent_t *arena_slab_alloc_hard(tsdn_t *tsdn, arena_t *arena,
    ...
    slab = extent_alloc_wrapper(tsdn, arena, r_extent_hooks, NULL,
                                bin_info->slab_size, 0, PAGE, &zero, &commit,
                                true);
    malloc_mutex_lock(tsdn, &arena->lock);
...
static extent_t *arena_slab_alloc(tsdn_t *tsdn, arena_t *arena, szind_t binind,
    ...
    slab = arena_extent_cache_alloc_locked(tsdn, arena, &extent_hooks, NULL,
                                           bin_info->slab_size, 0, PAGE, &zero,
                                           true);
    if (slab == NULL) {
        slab = arena_slab_alloc_hard(tsdn, arena, &extent_hooks, bin_info);
        if (slab == NULL) {
            return (NULL);
        }
    }
...
static extent_t *arena_bin_nonfull_slab_get(tsdn_t *tsdn, arena_t *arena,
                                            arena_bin_t *bin, szind_t binind) {
    extent_t *slab;
    const arena_bin_info_t *bin_info;
...
static void *arena_bin_malloc_hard(tsdn_t *tsdn, arena_t *arena,
                                   arena_bin_t *bin, szind_t binind) {
    const arena_bin_info_t *bin_info;
    extent_t *slab;
    ...
    if (bin->slabcur != NULL) {
        ...
        if (extent_slab_data_get(bin->slabcur)->nfree > 0) {
            void *ret = arena_slab_reg_alloc(tsdn, bin->slabcur, bin_info);
            if (slab != NULL) {
                /*
                 ...
                 */
                if (extent_slab_data_get(slab)->nfree == bin_info->nregs) {
...
void arena_tcache_fill_small(tsdn_t *tsdn, arena_t *arena, tcache_bin_t *tbin,
    ...
    for (i = 0,
        ...
        if ((slab = bin->slabcur) != NULL &&
            ...
            ptr = arena_slab_reg_alloc(tsdn, slab, &arena_bin_info[binind]);
        } else {
        ...
        if (config_fill && unlikely(opt_junk_alloc)) {
            arena_alloc_junk_small(ptr, &arena_bin_info[binind], true);
        }
...
void *arena_palloc(tsdn_t *tsdn, arena_t *arena, size_t usize, size_t alignment,
    ...
    if (usize <= SMALL_MAXCLASS &&
        ...
        ret = arena_malloc(tsdn, arena, usize, size2index(usize), zero, tcache,
                           true);
    } else {
...
static size_t arena_prof_demote(tsdn_t *tsdn, extent_t *extent,
                                const void *ptr) {
    cassert(config_prof);
    assert(ptr != NULL);
...
void arena_dalloc_promoted(tsdn_t *tsdn, extent_t *extent, void *ptr,
    ...
    if (usize <= tcache_maxclass) {
        tcache_dalloc_large(tsdn_tsd(tsdn), tcache, ptr, usize, slow_path);
    } else {
        large_dalloc(tsdn, extent);
    }
}

static void arena_dissociate_bin_slab(extent_t *slab, arena_bin_t *bin) {
...
bool arena_ralloc_no_move(tsdn_t *tsdn, extent_t *extent, void *ptr,
                          size_t oldsize, size_t size, size_t extra,
                          bool zero) {
...
static void *arena_ralloc_move_helper(tsdn_t *tsdn, arena_t *arena,
                                      ...
                                      tcache_t *tcache) {
    if (alignment == 0) {
        return (arena_malloc(tsdn, arena, usize, size2index(usize), zero,
                             tcache, true));
    }
...
void *arena_ralloc(tsdn_t *tsdn, arena_t *arena, extent_t *extent, void *ptr,
    ...
    if (likely(usize <= SMALL_MAXCLASS)) {
        ...
        if (!arena_ralloc_no_move(tsdn, extent, ptr, oldsize, usize, 0, zero)) {
            return (ptr);
        }
    }
    ...
    if (oldsize >= LARGE_MINCLASS && usize >= LARGE_MINCLASS) {
        return (
            large_ralloc(tsdn, arena, extent, usize, alignment, zero, tcache));
    }
     ...
     */
    ret = arena_ralloc_move_helper(tsdn, arena, usize, alignment, zero, tcache);
    if (ret == NULL) {
        return (NULL);
    }
...
static void arena_basic_stats_merge_locked(arena_t *arena, unsigned *nthreads,
                                           const char **dss,
                                           ssize_t *decay_time, size_t *nactive,
                                           size_t *ndirty) {
...
void arena_basic_stats_merge(tsdn_t *tsdn, arena_t *arena, unsigned *nthreads,
                             const char **dss, ssize_t *decay_time,
                             size_t *nactive, size_t *ndirty) {
    malloc_mutex_lock(tsdn, &arena->lock);
    arena_basic_stats_merge_locked(arena, nthreads, dss, decay_time, nactive,
                                   ndirty);
    malloc_mutex_unlock(tsdn, &arena->lock);
}

void arena_stats_merge(tsdn_t *tsdn, arena_t *arena, unsigned *nthreads,
                       const char **dss, ssize_t *decay_time, size_t *nactive,
    ...
    astats->resident +=
        base_resident + (((arena->nactive + arena->ndirty) << LG_PAGE));
    ...
    if (config_tcache) {
        ...
        ql_foreach(tcache, &arena->tcache_ql, link) {
            for (i = 0; i < nhbins; i++) {
                tbin = &tcache->tbins[i];
                astats->tcache_bytes += tbin->ncached * index2size(i);
            }
        }
    }
    malloc_mutex_unlock(tsdn, &arena->lock);
...
arena_t *arena_new(tsdn_t *tsdn, unsigned ind, extent_hooks_t *extent_hooks) {
    ...
    if (config_cache_oblivious) {
         ...
         * deterministic seed.
         */
        arena->offset_state = config_debug ? ind : (size_t)(uintptr_t)arena;
    }
    ...
    for (i = 0; i < NBINS; i++) {
        ...
        extent_init(&bin->slabs_full, arena, NULL, 0, 0, 0, false, false, false,
                    false);
        if (config_stats) {
