...
JEMALLOC_INLINE_C int prof_tctx_comp(const prof_tctx_t *a,
                                     const prof_tctx_t *b) {
...
rb_gen(static UNUSED, tctx_tree_, prof_tctx_tree_t, prof_tctx_t, tctx_link,
    ...
    JEMALLOC_INLINE_C
    int prof_gctx_comp(const prof_gctx_t *a, const prof_gctx_t *b) {
...
rb_gen(static UNUSED, gctx_tree_, prof_gctx_tree_t, prof_gctx_t, dump_link,
    ...
    JEMALLOC_INLINE_C
    int prof_tdata_comp(const prof_tdata_t *a, const prof_tdata_t *b) {
    int ret;
...
void prof_malloc_sample_object(tsdn_t *tsdn, extent_t *extent, const void *ptr,
...
JEMALLOC_INLINE_C void prof_enter(tsd_t *tsd, prof_tdata_t *tdata) {
    cassert(config_prof);
...
JEMALLOC_INLINE_C void prof_leave(tsd_t *tsd, prof_tdata_t *tdata) {
    cassert(config_prof);
    assert(tdata == prof_tdata_get(tsd, false));
...
static prof_gctx_t *prof_gctx_create(tsdn_t *tsdn, prof_bt_t *bt) {
    ...
    prof_gctx_t *gctx =
        (prof_gctx_t *)iallocztm(tsdn, size, size2index(size), false, NULL,
...
static void prof_gctx_try_destroy(tsd_t *tsd, prof_tdata_t *tdata_self,
    ...
    if (tctx_tree_empty(&gctx->tctxs) && gctx->nlimbo == 1) {
        ...
        idalloctm(tsd_tsdn(tsd), iealloc(tsd_tsdn(tsd), gctx), gctx, NULL, true,
                  true);
    } else {
        /*
...
static void prof_tctx_destroy(tsd_t *tsd, prof_tctx_t *tctx) {
    ...
    if (destroy_tctx) {
        idalloctm(tsd_tsdn(tsd), iealloc(tsd_tsdn(tsd), tctx), tctx, NULL, true,
                  true);
    }
}
...
static bool prof_lookup_global(tsd_t *tsd, prof_bt_t *bt, prof_tdata_t *tdata,
    ...
    if (ckh_search(&bt2gctx, bt, &btkey.v, &gctx.v)) {
        ...
        if (ckh_insert(tsd, &bt2gctx, btkey.v, gctx.v)) {
            /* OOM. */
            prof_leave(tsd, tdata);
            idalloctm(tsd_tsdn(tsd), iealloc(tsd_tsdn(tsd), gctx.v), gctx.v,
                      NULL, true, true);
            return (true);
        }
        new_gctx = true;
...
prof_tctx_t *prof_lookup(tsd_t *tsd, prof_bt_t *bt) {
    ...
    if (not_found) {
        ...
        if (error) {
            ...
            idalloctm(tsd_tsdn(tsd), iealloc(tsd_tsdn(tsd), ret.v), ret.v, NULL,
                      true, true);
            return (NULL);
        }
        malloc_mutex_lock(tsd_tsdn(tsd), gctx->lock);
...
#endif
static int prof_dump_open(bool propagate_err, const char *filename) {
    int fd;
...
static prof_tctx_t *prof_tctx_dump_iter(prof_tctx_tree_t *tctxs,
    ...
    case prof_tctx_state_dumping:
    case prof_tctx_state_purgatory:
        if (prof_dump_printf(arg->propagate_err, "  t%" FMTu64 ": %" FMTu64
                                                 ": %" FMTu64 " [%" FMTu64 ": "
                                                 "%" FMTu64 "]\n",
                             tctx->thr_uid, tctx->dump_cnts.curobjs,
                             tctx->dump_cnts.curbytes,
        ...
        malloc_mutex_lock(tsd_tsdn(tsd), gctx->lock);
        {
            ...
            do {
                ...
                if (to_destroy != NULL) {
                    ...
                    idalloctm(tsd_tsdn(tsd), iealloc(tsd_tsdn(tsd), to_destroy),
...
static prof_tdata_t *prof_tdata_dump_iter(prof_tdata_tree_t *tdatas,
    ...
    if (prof_dump_printf(
            propagate_err, "  t%" FMTu64 ": %" FMTu64 ": %" FMTu64 " [%" FMTu64
                           ": %" FMTu64 "]%s%s\n",
...
#endif
static bool prof_dump_header(tsdn_t *tsdn, bool propagate_err,
                             const prof_cnt_t *cnt_all) {
    bool ret;

    if (prof_dump_printf(propagate_err, "heap_v2/%" FMTu64 "\n"
                                        "  t*: %" FMTu64 ": %" FMTu64
                                        " [%" FMTu64 ": %" FMTu64 "]\n",
                         ((uint64_t)1U << lg_prof_sample), cnt_all->curobjs,
                         cnt_all->curbytes, cnt_all->accumobjs,
                         cnt_all->accumbytes)) {
        return (true);
    }
...
static bool prof_dump_gctx(tsdn_t *tsdn, bool propagate_err, prof_gctx_t *gctx,
    ...
    if (prof_dump_printf(propagate_err, "\n"
                         ...
                         gctx->cnt_summed.accumobjs,
                         gctx->cnt_summed.accumbytes)) {
        ret = true;
        goto label_return;
    }
...
JEMALLOC_INLINE_C uint64_t prof_thr_uid_alloc(tsdn_t *tsdn) {
    uint64_t thr_uid;
...
static prof_tdata_t *prof_tdata_init_impl(tsd_t *tsd, uint64_t thr_uid,
    ...
    tdata = (prof_tdata_t *)iallocztm(
        tsd_tsdn(tsd), sizeof(prof_tdata_t), size2index(sizeof(prof_tdata_t)),
...
static void prof_tdata_destroy_locked(tsd_t *tsd, prof_tdata_t *tdata,
    ...
    if (tdata->thread_name != NULL) {
        idalloctm(tsd_tsdn(tsd), iealloc(tsd_tsdn(tsd), tdata->thread_name),
                  tdata->thread_name, NULL, true, true);
    }
    ckh_delete(tsd, &tdata->bt2tctx);
    idalloctm(tsd_tsdn(tsd), iealloc(tsd_tsdn(tsd), tdata), tdata, NULL, true,
              true);
}
