...
const extent_hooks_t extent_hooks_default = {
    extent_alloc_default, extent_dalloc_default, extent_commit_default,
...
ph_gen(, extent_heap_, extent_heap_t, extent_t, ph_link, extent_snad_comp)
    ...
    static void extent_heaps_insert(tsdn_t *tsdn,
                                    extent_heap_t extent_heaps[NPSIZES + 1],
                                    extent_t *extent) {
...
static void extent_heaps_remove(tsdn_t *tsdn,
                                extent_heap_t extent_heaps[NPSIZES + 1],
                                extent_t *extent) {
    size_t psz = extent_size_quantize_floor(extent_size_get(extent));
    pszind_t pind = psz2ind(psz);
...
static bool extent_rtree_acquire(tsdn_t *tsdn, rtree_ctx_t *rtree_ctx,
                                 const extent_t *extent, bool dependent,
                                 bool init_missing, rtree_elm_t **r_elm_a,
                                 rtree_elm_t **r_elm_b) {
    *r_elm_a = rtree_elm_acquire(tsdn, &extents_rtree, rtree_ctx,
                                 (uintptr_t)extent_base_get(extent), dependent,
                                 init_missing);
...
static void extent_rtree_write_acquired(tsdn_t *tsdn, rtree_elm_t *elm_a,
                                        rtree_elm_t *elm_b,
...
static void extent_rtree_release(tsdn_t *tsdn, rtree_elm_t *elm_a,
                                 rtree_elm_t *elm_b) {
...
static void extent_gprof_add(tsdn_t *tsdn, const extent_t *extent) {
    cassert(config_prof);
...
static void extent_gprof_sub(tsdn_t *tsdn, const extent_t *extent) {
    cassert(config_prof);
...
static bool extent_register(tsdn_t *tsdn, const extent_t *extent) {
...
static void extent_reregister(tsdn_t *tsdn, const extent_t *extent) {
...
static void extent_deregister(tsdn_t *tsdn, extent_t *extent) {
    ...
    extent_rtree_acquire(tsdn, rtree_ctx, extent, true, false, &elm_a, &elm_b);
 ...
 * best fits.
 */
static extent_t *extent_first_best_fit(tsdn_t *tsdn, arena_t *arena,
                                       extent_heap_t extent_heaps[NPSIZES + 1],
                                       size_t size) {
    pszind_t pind, i;
...
static void extent_leak(tsdn_t *tsdn, arena_t *arena,
                        extent_hooks_t **r_extent_hooks, bool cache,
                        extent_t *extent) {
    /*
     ...
     */
    if (cache) {
        if (extent_purge_lazy_wrapper(tsdn, arena, r_extent_hooks, extent, 0,
                                      extent_size_get(extent))) {
            extent_purge_forced_wrapper(tsdn, arena, r_extent_hooks, extent, 0,
                                        extent_size_get(extent));
        }
    }
...
static extent_t *extent_recycle(tsdn_t *tsdn, arena_t *arena,
                                extent_hooks_t **r_extent_hooks,
                                extent_heap_t extent_heaps[NPSIZES + 1],
                                bool locked, bool cache, void *new_addr,
    ...
    if (config_debug && new_addr != NULL) {
        ...
        prev = extent_lookup(tsdn, (void *)((uintptr_t)new_addr - PAGE), false);
    ...
    if (new_addr != NULL) {
        ...
        if (elm != NULL) {
            extent = rtree_elm_read_acquired(tsdn, &extents_rtree, elm);
            if (extent != NULL) {
        ...
        extent = extent_first_best_fit(tsdn, arena, extent_heaps, alloc_size);
    }
    if (extent == NULL) {
        if (!locked) {
    ...
    if (leadsize != 0) {
        ...
        extent =
            extent_split_wrapper(tsdn, arena, r_extent_hooks, lead, leadsize,
                                 leadsize, size + trailsize, usize + trailsize);
        if (extent == NULL) {
    ...
    if (trailsize != 0) {
        extent_t *trail =
            extent_split_wrapper(tsdn, arena, r_extent_hooks, extent, size,
                                 usize, trailsize, trailsize);
        if (trail == NULL) {
            extent_deregister(tsdn, extent);
            extent_leak(tsdn, arena, r_extent_hooks, cache, extent);
            if (!locked) {
                malloc_mutex_unlock(tsdn, &arena->extents_mtx);
            }
            return (NULL);
    ...
    if (*commit && !extent_committed_get(extent)) {
        if (extent_commit_wrapper(tsdn, arena, r_extent_hooks, extent, 0,
                                  extent_size_get(extent))) {
            if (!locked) {
                malloc_mutex_unlock(tsdn, &arena->extents_mtx);
            }
    ...
    if (*zero) {
        if (!extent_zeroed_get(extent)) {
            ...
            size_t i;
            size_t *p = (size_t *)(uintptr_t)extent_addr_get(extent);
...
static extent_t *extent_alloc_cache_impl(tsdn_t *tsdn, arena_t *arena,
                                         extent_hooks_t **r_extent_hooks,
                                         bool locked, void *new_addr,
                                         size_t usize, size_t pad,
    ...
    extent = extent_recycle(tsdn, arena, r_extent_hooks, arena->extents_cached,
                            locked, true, new_addr, usize, pad, alignment, zero,
                            commit, slab);
    return (extent);
}
...
extent_t *extent_alloc_cache_locked(tsdn_t *tsdn, arena_t *arena,
                                    extent_hooks_t **r_extent_hooks,
                                    void *new_addr, size_t usize, size_t pad,
                                    size_t alignment, bool *zero, bool *commit,
                                    bool slab) {
    malloc_mutex_assert_owner(tsdn, &arena->extents_mtx);

    return (extent_alloc_cache_impl(tsdn, arena, r_extent_hooks, true, new_addr,
...
extent_t *extent_alloc_cache(tsdn_t *tsdn, arena_t *arena,
    ...
    return (extent_alloc_cache_impl(tsdn, arena, r_extent_hooks, false,
                                    new_addr, usize, pad, alignment, zero,
                                    commit, slab));
}
...
static void *extent_alloc_default(extent_hooks_t *extent_hooks, void *new_addr,
    ...
    return (extent_alloc_default_impl(tsdn, arena, new_addr, size, alignment,
                                      zero, commit));
}

static void extent_retain(tsdn_t *tsdn, arena_t *arena,
                          extent_hooks_t **r_extent_hooks, extent_t *extent) {
    if (config_stats) {
        arena->stats.retained += extent_size_get(extent);
    }
    extent_record(tsdn, arena, r_extent_hooks, arena->extents_retained, false,
                  extent);
}
...
static extent_t *extent_grow_retained(tsdn_t *tsdn, arena_t *arena,
    ...
    zeroed = false;
    committed = false;
    ptr = extent_alloc_core(tsdn, arena, new_addr, alloc_size, PAGE, &zeroed,
                            &committed, arena->dss_prec);
...
static extent_t *extent_alloc_retained(tsdn_t *tsdn, arena_t *arena,
    ...
    extent = extent_recycle(tsdn, arena, r_extent_hooks,
                            arena->extents_retained, false, false, new_addr,
                            usize, pad, alignment, zero, commit, slab);
    ...
    if (!config_munmap && extent == NULL) {
        extent =
            extent_grow_retained(tsdn, arena, r_extent_hooks, new_addr, usize,
...
static extent_t *extent_alloc_wrapper_hard(tsdn_t *tsdn, arena_t *arena,
    ...
    if (*r_extent_hooks == &extent_hooks_default) {
        ...
        addr = extent_alloc_default_impl(tsdn, arena, new_addr, size, alignment,
                                         zero, commit);
    } else {
        addr = (*r_extent_hooks)
                   ->alloc(*r_extent_hooks, new_addr, size, alignment, zero,
    ...
    extent_init(extent, arena, addr, size, usize, arena_extent_sn_next(arena),
                true, zero, commit, slab);
    if (pad != 0) {
        extent_addr_randomize(tsdn, extent, alignment);
    }
...
extent_t *extent_alloc_wrapper(tsdn_t *tsdn, arena_t *arena,
    ...
    extent = extent_alloc_retained(tsdn, arena, r_extent_hooks, new_addr, usize,
                                   pad, alignment, zero, commit, slab);
    if (extent == NULL) {
        extent = extent_alloc_wrapper_hard(tsdn, arena, r_extent_hooks,
                                           new_addr, usize, pad, alignment,
                                           zero, commit, slab);
    }
...
static bool extent_can_coalesce(const extent_t *a, const extent_t *b) {
...
static void extent_try_coalesce(tsdn_t *tsdn, arena_t *arena,
    ...
    if (extent_merge_wrapper(tsdn, arena, r_extent_hooks, a, b)) {
        ...
        arena_extent_cache_maybe_insert(tsdn, extent_arena_get(b), b, cache);
        return;
    }
...
static bool extent_commit_default(extent_hooks_t *extent_hooks, void *addr,
    ...
    return (
        pages_commit((void *)((uintptr_t)addr + (uintptr_t)offset), length));
}
...
static bool extent_decommit_default(extent_hooks_t *extent_hooks, void *addr,
    ...
    return (
        pages_decommit((void *)((uintptr_t)addr + (uintptr_t)offset), length));
}
...
extent_t *extent_split_wrapper(tsdn_t *tsdn, arena_t *arena,
                               extent_hooks_t **r_extent_hooks,
                               extent_t *extent, size_t size_a, size_t usize_a,
...

    {
        ...
        extent_init(&lead, arena, extent_addr_get(extent), size_a, usize_a,
                    extent_sn_get(extent), extent_active_get(extent),
    ...
    extent_init(trail, arena,
                ...
                usize_b, extent_sn_get(extent), extent_active_get(extent),
                extent_zeroed_get(extent), extent_committed_get(extent),
                extent_slab_get(extent));
    if (extent_rtree_acquire(tsdn, rtree_ctx, trail, false, true, &trail_elm_a,
                             &trail_elm_b)) {
        goto label_error_c;
    }

    if ((*r_extent_hooks)
            ->split(*r_extent_hooks, extent_base_get(extent), size_a + size_b,
                    size_a, size_b, extent_committed_get(extent),
                    arena_ind_get(arena))) {
        goto label_error_d;
    }
...
bool extent_merge_wrapper(tsdn_t *tsdn, arena_t *arena,
     ...
     */
    extent_rtree_acquire(tsdn, rtree_ctx, a, true, false, &a_elm_a, &a_elm_b);
    extent_rtree_acquire(tsdn, rtree_ctx, b, true, false, &b_elm_a, &b_elm_b);
...
bool extent_boot(void) {
    if (rtree_new(&extents_rtree,
                  (unsigned)((ZU(1) << (LG_SIZEOF_PTR + 3)) - LG_PAGE))) {
        return (true);
    }
