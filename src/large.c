    ...
    if (diff != 0) {
        extent_t *trail =
            extent_split_wrapper(tsdn, arena, &extent_hooks, extent,
                                 usize + large_pad, usize, diff, diff);
        if (trail == NULL) {
            return (true);
        }
...
static bool large_ralloc_no_move_expand(tsdn_t *tsdn, extent_t *extent,
    ...
    if ((trail = arena_extent_cache_alloc(
             tsdn, arena, &extent_hooks, extent_past_get(extent), trailsize,
             CACHELINE, &is_zeroed_trail)) == NULL) {
        bool commit = true;
        if ((trail = extent_alloc_wrapper(
                 tsdn, arena, &extent_hooks, extent_past_get(extent), trailsize,
                 0, CACHELINE, &is_zeroed_trail, &commit, false)) == NULL) {
            return (true);
        }
    }
    ...
    if (zero || (config_fill && unlikely(opt_zero))) {
        ...
        if (!is_zeroed_trail) {
            memset((void *)((uintptr_t)extent_addr_get(extent) + oldusize), 0,
                   usize - oldusize);
        }
    } else if (config_fill && unlikely(opt_junk_alloc)) {
...
bool large_ralloc_no_move(tsdn_t *tsdn, extent_t *extent, size_t usize_min,
    ...
    assert(extent_usize_get(extent) >= LARGE_MINCLASS &&
           usize_max >= LARGE_MINCLASS);
    ...
    if (usize_max > extent_usize_get(extent)) {
        ...
        if (usize_min < usize_max && usize_min > extent_usize_get(extent) &&
     ...
     * the new size.
     */
    if (extent_usize_get(extent) >= usize_min &&
        extent_usize_get(extent) <= usize_max) {
        arena_decay_tick(tsdn, extent_arena_get(extent));
        return (false);
    }
...
void *large_ralloc(tsdn_t *tsdn, arena_t *arena, extent_t *extent, size_t usize,
    ...
    copysize =
        (usize < extent_usize_get(extent)) ? usize : extent_usize_get(extent);
    memcpy(ret, extent_addr_get(extent), copysize);
    isdalloct(tsdn, extent, extent_addr_get(extent), extent_usize_get(extent),
              tcache, true);
    return (ret);
}
 ...
 */
static void large_dalloc_impl(tsdn_t *tsdn, extent_t *extent,
                              bool junked_locked) {
    arena_t *arena;
