...
static void *base_map(extent_hooks_t *extent_hooks, unsigned ind, size_t size) {
    void *addr;
    bool zero = true;
    bool commit = true;
    ...
    if (extent_hooks == &extent_hooks_default) {
        ...
        addr = extent_hooks->alloc(extent_hooks, NULL, size, PAGE, &zero,
                                   &commit, ind);
    }
...
static void base_unmap(extent_hooks_t *extent_hooks, unsigned ind, void *addr,
                       size_t size) {
    /*
...
static void *base_extent_bump_alloc_helper(extent_t *extent, size_t *gap_size,
    ...
    *gap_size =
    ...
    extent_init(extent, NULL,
                (void *)((uintptr_t)extent_addr_get(extent) + *gap_size + size),
                extent_size_get(extent) - *gap_size - size, 0,
                extent_sn_get(extent), true, true, true, false);
    return (ret);
}

static void base_extent_bump_alloc_post(tsdn_t *tsdn, base_t *base,
                                        extent_t *extent, size_t gap_size,
                                        void *addr, size_t size) {
    if (extent_size_get(extent) > 0) {
         ...
         */
        szind_t index_floor = size2index(extent_size_get(extent) + 1) - 1;
...
static void *base_extent_bump_alloc(tsdn_t *tsdn, base_t *base,
                                    extent_t *extent, size_t size,
                                    size_t alignment) {
    void *ret;
    size_t gap_size;
 ...
 */
static base_block_t *base_block_alloc(extent_hooks_t *extent_hooks,
                                      unsigned ind, size_t *extent_sn_next,
                                      size_t size, size_t alignment) {
    base_block_t *block;
...
base_t *base_new(tsdn_t *tsdn, unsigned ind, extent_hooks_t *extent_hooks) {
    ...
    extent_sn_next = 0;
    block = base_block_alloc(extent_hooks, ind, &extent_sn_next, sizeof(base_t),
                             QUANTUM);
    if (block == NULL) {
        return (NULL);
    }
    ...
    base = (base_t *)base_extent_bump_alloc_helper(&block->extent, &gap_size,
                                                   base_size, base_alignment);
    base->ind = ind;
...
void base_delete(base_t *base) {
    ...
    do {
        ...
        next = block->next;
        base_unmap(extent_hooks, base_ind_get(base), block, block->size);
    } while (next != NULL);
}
