...
static unsigned hmin(unsigned ha, unsigned hb) {
    return (ha < hb ? ha : hb);
}
...
bool rtree_new(rtree_t *rtree, unsigned bits) {
    ...
    rtree->levels[0].bits = (height > 1) ? RTREE_BITS_PER_LEVEL : bits_in_leaf;
    rtree->levels[0].cumbits = rtree->levels[0].bits;
    /* Interior levels. */
    for (i = 1; i < height - 1; i++) {
        rtree->levels[i].subtree = NULL;
        rtree->levels[i].bits = RTREE_BITS_PER_LEVEL;
        rtree->levels[i].cumbits =
            rtree->levels[i - 1].cumbits + RTREE_BITS_PER_LEVEL;
    }
    /* Leaf level. */
    if (height > 1) {
...
#endif
static rtree_elm_t *rtree_node_alloc(tsdn_t *tsdn, rtree_t *rtree,
                                     size_t nelms) {
    return ((rtree_elm_t *)base_alloc(tsdn, b0get(),
                                      nelms * sizeof(rtree_elm_t), CACHELINE));
}
#ifdef JEMALLOC_JET
#undef rtree_node_alloc
...
#endif
UNUSED static void rtree_node_dalloc(tsdn_t *tsdn, rtree_t *rtree,
                                     rtree_elm_t *node) {
...
static void rtree_delete_subtree(tsdn_t *tsdn, rtree_t *rtree,
                                 rtree_elm_t *node, unsigned level) {
    if (level + 1 < rtree->height) {
        ...
        for (i = 0; i < nchildren; i++) {
            ...
            if (child != NULL) {
                rtree_delete_subtree(tsdn, rtree, child, level + 1);
            }
        }
    }
...
static rtree_elm_t *rtree_node_init(tsdn_t *tsdn, rtree_t *rtree,
                                    unsigned level, rtree_elm_t **elmp) {
    rtree_elm_t *node;
    ...
    if (node == NULL) {
        node =
            rtree_node_alloc(tsdn, rtree, ZU(1) << rtree->levels[level].bits);
        if (node == NULL) {
...
rtree_elm_t *rtree_subtree_read_hard(tsdn_t *tsdn, rtree_t *rtree,
                                     unsigned level) {
    return (rtree_node_init(tsdn, rtree, level, &rtree->levels[level].subtree));
}
...
static int rtree_elm_witness_comp(const witness_t *a, void *oa,
...
static witness_t *rtree_elm_witness_alloc(tsd_t *tsd, uintptr_t key,
    ...
    witness = NULL;
    for (i = 0; i < sizeof(rtree_elm_witness_tsd_t) / sizeof(witness_t); i++) {
        ...
        if (rew->elm == NULL && witness == NULL) {
            rew->elm = elm;
            witness = &rew->witness;
            witness_init(witness, "rtree_elm", WITNESS_RANK_RTREE_ELM,
                         rtree_elm_witness_comp, (void *)key);
        }
    }
    assert(witness != NULL);
    return (witness);
...
static witness_t *rtree_elm_witness_find(tsd_t *tsd, const rtree_elm_t *elm) {
    size_t i;
    rtree_elm_witness_tsd_t *witnesses = tsd_rtree_elm_witnessesp_get(tsd);
...
static void rtree_elm_witness_dalloc(tsd_t *tsd, witness_t *witness,
                                     const rtree_elm_t *elm) {
    size_t i;
    rtree_elm_witness_tsd_t *witnesses = tsd_rtree_elm_witnessesp_get(tsd);
    ...
    for (i = 0; i < sizeof(rtree_elm_witness_tsd_t) / sizeof(witness_t); i++) {
        ...
        if (rew->elm == elm) {
            rew->elm = NULL;
            witness_init(&rew->witness, "rtree_elm", WITNESS_RANK_RTREE_ELM,
                         rtree_elm_witness_comp, NULL);
            return;
        }
    }
    not_reached();
