...
static unsigned stats_arenas_i2a_impl(size_t i, bool compat, bool validate) {
    unsigned a;
    ...
    default:
        if (compat && i == ctl_stats->narenas) {
             ...
             * initialized stats.
             */
            assert(i < ctl_stats->narenas ||
                   (!validate && i == ctl_stats->narenas));
            a = (unsigned)i + 2;
        }
        break;
    }
...
static unsigned stats_arenas_i2a(size_t i) {
    return (stats_arenas_i2a_impl(i, true, false));
}

static ctl_arena_stats_t *stats_arenas_i_impl(tsdn_t *tsdn, size_t i,
                                              bool compat, bool init) {
    ...
    if (init && ret == NULL) {
        ret = (ctl_arena_stats_t *)base_alloc(
            tsdn, b0get(), sizeof(ctl_arena_stats_t), QUANTUM);
        if (ret == NULL) {
            return (NULL);
        }
        ret->arena_ind = (unsigned)i;
        ctl_stats->arenas[stats_arenas_i2a_impl(i, compat, false)] = ret;
    }

    assert(ret == NULL ||
           stats_arenas_i2a(ret->arena_ind) == stats_arenas_i2a(i));
    return (ret);
}

static ctl_arena_stats_t *stats_arenas_i(size_t i) {
...
static void ctl_arena_clear(ctl_arena_stats_t *astats) {
    ...
    if (config_stats) {
        ...
        memset(astats->lstats, 0,
               (NSIZES - NBINS) * sizeof(malloc_large_stats_t));
    }
}
...
static void ctl_arena_stats_amerge(tsdn_t *tsdn, ctl_arena_stats_t *cstats,
    ...
    if (config_stats) {
        arena_stats_merge(tsdn, arena, &cstats->nthreads, &cstats->dss,
                          &cstats->decay_time, &cstats->pactive,
                          &cstats->pdirty, &cstats->astats, cstats->bstats,
                          cstats->lstats);

        for (i = 0; i < NBINS; i++) {
            cstats->allocated_small +=
                cstats->bstats[i].curregs * index2size(i);
            cstats->nmalloc_small += cstats->bstats[i].nmalloc;
            cstats->ndalloc_small += cstats->bstats[i].ndalloc;
            cstats->nrequests_small += cstats->bstats[i].nrequests;
        }
    } else {
        arena_basic_stats_merge(tsdn, arena, &cstats->nthreads, &cstats->dss,
                                &cstats->decay_time, &cstats->pactive,
                                &cstats->pdirty);
    }
}

static void ctl_arena_stats_sdmerge(ctl_arena_stats_t *sdstats,
                                    ctl_arena_stats_t *astats, bool destroyed) {
    unsigned i;
...
static void ctl_arena_refresh(tsdn_t *tsdn, arena_t *arena,
                              ctl_arena_stats_t *sdstats, unsigned i,
                              bool destroyed) {
...
static unsigned ctl_arena_init(tsdn_t *tsdn, extent_hooks_t *extent_hooks) {
    unsigned arena_ind;
...
static void ctl_refresh(tsdn_t *tsdn) {
    ...
    if (config_stats) {
        ctl_stats->allocated =
            sstats->allocated_small + sstats->astats.allocated_large;
        ctl_stats->active = (sstats->pactive << LG_PAGE);
        ctl_stats->metadata = sstats->astats.base + sstats->astats.internal;
...
static bool ctl_init(tsdn_t *tsdn) {
    ...
    if (!ctl_initialized) {
         ...
         */
        if ((sstats = stats_arenas_i_impl(tsdn, MALLCTL_ARENAS_ALL, false,
                                          true)) == NULL) {
            ret = true;
            goto label_return;
        }
        ...
        if ((dstats = stats_arenas_i_impl(tsdn, MALLCTL_ARENAS_DESTROYED, false,
                                          true)) == NULL) {
            ret = true;
            goto label_return;
        }
...
static int thread_arena_ctl(tsd_t *tsd, const size_t *mib, size_t miblen,
    ...
    if (newind != oldind) {
        ...
        if (config_tcache) {
            ...
            if (tcache != NULL) {
                tcache_arena_reassociate(tsd_tsdn(tsd), tcache, oldarena,
                                         newarena);
            }
        }
    }
...
static int arena_i_decay_time_ctl(tsd_t *tsd, const size_t *mib, size_t miblen,
                                  void *oldp, size_t *oldlenp, void *newp,
                                  size_t newlen) {
    int ret;
    unsigned arena_ind;
    ...
    if (newp != NULL) {
        ...
        if (arena_decay_time_set(tsd_tsdn(tsd), arena, *(ssize_t *)newp)) {
            ret = EFAULT;
            goto label_return;
        }
    }
...
static int arena_i_extent_hooks_ctl(tsd_t *tsd, const size_t *mib,
    ...
    if (arena_ind < narenas_total_get() &&
        ...
        if (newp != NULL) {
            ...
            old_extent_hooks = extent_hooks_set(arena, new_extent_hooks);
...
static int arenas_decay_time_ctl(tsd_t *tsd, const size_t *mib, size_t miblen,
                                 void *oldp, size_t *oldlenp, void *newp,
                                 size_t newlen) {
    int ret;
...
CTL_RO_NL_GEN(arenas_lextent_i_size, index2size(NBINS + (szind_t)mib[2]),
              size_t)
static const ctl_named_node_t *arenas_lextent_i_index(tsdn_t *tsdn,
                                                      const size_t *mib,
                                                      size_t miblen, size_t i) {
    if (i > NSIZES - NBINS) {
        return (NULL);
...
static int arenas_create_ctl(tsd_t *tsd, const size_t *mib, size_t miblen,
    ...
    if ((arena_ind = ctl_arena_init(tsd_tsdn(tsd), extent_hooks)) == UINT_MAX) {
        ret = EAGAIN;
        goto label_return;
    }
...
CTL_RO_GEN(stats_arenas_i_decay_time, stats_arenas_i(mib[2])->decay_time,
           ssize_t)
CTL_RO_GEN(stats_arenas_i_nthreads, stats_arenas_i(mib[2])->nthreads, unsigned)
...
CTL_RO_CGEN(config_stats, stats_arenas_i_large_nrequests,
            stats_arenas_i(mib[2])->astats.nmalloc_large,
