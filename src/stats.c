...
static void stats_arena_bins_print(void (*write_cb)(void *, const char *),
                                   void *cbopaque, bool json, bool large,
                                   unsigned i) {
    size_t page;
    ...
    if (json) {
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\t\"bins\": [\n");
    } else {
    ...
    for (j = 0, in_gap = false; j < nbins; j++) {
        ...
        if (json) {
            malloc_cprintf(write_cb, cbopaque,
                           ...
                           "\t\t\t\t\t\t\"curregs\": %zu,\n"
                           "\t\t\t\t\t\t\"nrequests\": %" FMTu64 ",\n",
                           nmalloc, ndalloc, curregs, nrequests);
            if (config_tcache) {
                malloc_cprintf(write_cb, cbopaque,
                               "\t\t\t\t\t\t\"nfills\": %" FMTu64 ",\n"
                               "\t\t\t\t\t\t\"nflushes\": %" FMTu64 ",\n",
            ...
            malloc_cprintf(write_cb, cbopaque,
                           ...
                           "\t\t\t\t\t\t\"curslabs\": %zu\n"
                           "\t\t\t\t\t}%s\n",
                           nreslabs, curslabs, (j + 1 < nbins) ? "," : "");
        } else if (!in_gap) {
            size_t availregs, milli;
            char util[6]; /* "x.yyy". */

            availregs = nregs * curslabs;
            milli = (availregs != 0) ? (1000 * curregs) / availregs : 1000;
            assert(milli <= 1000);
            if (milli < 10) {
                malloc_snprintf(util, sizeof(util), "0.00%zu", milli);
            } else if (milli < 100) {
                malloc_snprintf(util, sizeof(util), "0.0%zu", milli);
            } else if (milli < 1000) {
                malloc_snprintf(util, sizeof(util), "0.%zu", milli);
            } else {
            ...
            if (config_tcache) {
                malloc_cprintf(write_cb, cbopaque,
                               ...
                               " %12" FMTu64 " %12zu"
                               " %12zu %4u %3zu %-5s %12" FMTu64 " %12" FMTu64
                               " %12" FMTu64 " %12" FMTu64 "\n",
                               reg_size, j, curregs * reg_size, nmalloc,
                               ndalloc, nrequests, curregs, curslabs, nregs,
                               slab_size / page, util, nfills, nflushes, nslabs,
                               nreslabs);
            } else {
                malloc_cprintf(write_cb, cbopaque,
                               "%20zu %3u %12zu %12" FMTu64 " %12" FMTu64
                               ...
                               reg_size, j, curregs * reg_size, nmalloc,
                               ndalloc, nrequests, curregs, curslabs, nregs,
                               slab_size / page, util, nslabs, nreslabs);
            }
        }
    }
    if (json) {
...
static void stats_arena_lextents_print(void (*write_cb)(void *, const char *),
    ...
    if (json) {
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\t\"lextents\": [\n");
    } else {
        malloc_cprintf(write_cb, cbopaque,
    ...
    for (j = 0, in_gap = false; j < nlextents; j++) {
        ...
        if (json) {
            malloc_cprintf(write_cb, cbopaque,
                           ...
                           "\t\t\t\t\t\t\"curlextents\": %zu\n"
                           "\t\t\t\t\t}%s\n",
                           curlextents, (j + 1 < nlextents) ? "," : "");
        } else if (!in_gap) {
            malloc_cprintf(write_cb, cbopaque,
                           "%20zu %3u %12zu %12" FMTu64 " %12" FMTu64
                           " %12" FMTu64 " %12zu\n",
                           lextent_size, nbins + j, curlextents * lextent_size,
                           nmalloc, ndalloc, nrequests, curlextents);
        }
    }
    if (json) {
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\t]\n");
...
static void stats_arena_print(void (*write_cb)(void *, const char *),
                              void *cbopaque, bool json, unsigned i, bool bins,
                              bool large) {
    unsigned nthreads;
    const char *dss;
    ssize_t decay_time;
    ...
    if (json) {
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\t\"nthreads\": %u,\n",
                       nthreads);
    } else {
    ...
    if (json) {
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\t\"dss\": \"%s\",\n", dss);
    } else {
    ...
    if (json) {
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\t\"decay_time\": %zd,\n",
                       decay_time);
    } else {
        if (decay_time >= 0) {
    ...
    if (json) {
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\t\"pactive\": %zu,\n",
                       pactive);
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\t\"pdirty\": %zu,\n",
                       pdirty);
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\t\"npurge\": %" FMTu64 ",\n",
                       npurge);
        malloc_cprintf(write_cb, cbopaque,
                       "\t\t\t\t\"nmadvise\": %" FMTu64 ",\n", nmadvise);
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\t\"purged\": %" FMTu64 ",\n",
                       purged);
    } else {
        malloc_cprintf(write_cb, cbopaque,
                       "purging: dirty: %zu, sweeps: %" FMTu64
                       ", madvises: %" FMTu64 ", purged: %" FMTu64 "\n",
                       pdirty, npurge, nmadvise, purged);
    }

    CTL_M2_GET("stats.arenas.0.small.allocated", i, &small_allocated, size_t);
    ...
    CTL_M2_GET("stats.arenas.0.small.nrequests", i, &small_nrequests, uint64_t);
    if (json) {
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\t\"small\": {\n");
        ...
        malloc_cprintf(write_cb, cbopaque,
                       ...
                       "      ndalloc    nrequests\n");
        malloc_cprintf(
            write_cb, cbopaque, "small:                   %12zu %12" FMTu64
                                " %12" FMTu64 " %12" FMTu64 "\n",
            small_allocated, small_nmalloc, small_ndalloc, small_nrequests);
    }

    CTL_M2_GET("stats.arenas.0.large.allocated", i, &large_allocated, size_t);
    CTL_M2_GET("stats.arenas.0.large.nmalloc", i, &large_nmalloc, uint64_t);
    CTL_M2_GET("stats.arenas.0.large.ndalloc", i, &large_ndalloc, uint64_t);
    CTL_M2_GET("stats.arenas.0.large.nrequests", i, &large_nrequests, uint64_t);
    if (json) {
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\t\"large\": {\n");
        ...
        malloc_cprintf(
            write_cb, cbopaque, "large:                   %12zu %12" FMTu64
                                " %12" FMTu64 " %12" FMTu64 "\n",
            large_allocated, large_nmalloc, large_ndalloc, large_nrequests);
        malloc_cprintf(
            write_cb, cbopaque, "total:                   %12zu %12" FMTu64
                                " %12" FMTu64 " %12" FMTu64 "\n",
    ...
    if (json) {
        ...
        malloc_cprintf(write_cb, cbopaque, "retained:                %12zu\n",
                       retained);
    }

    CTL_M2_GET("stats.arenas.0.base", i, &base, size_t);
    if (json) {
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\t\"base\": %zu,\n", base);
    } else {
        malloc_cprintf(write_cb, cbopaque, "base:                    %12zu\n",
                       base);
    }

    CTL_M2_GET("stats.arenas.0.internal", i, &internal, size_t);
    if (json) {
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\t\"internal\": %zu,\n",
                       internal);
    } else {
        malloc_cprintf(write_cb, cbopaque, "internal:                %12zu\n",
                       internal);
    }
    ...
    if (config_tcache) {
        ...
        if (json) {
            malloc_cprintf(write_cb, cbopaque, "\t\t\t\t\"tcache\": %zu,\n",
                           tcache_bytes);
        } else {
            malloc_cprintf(write_cb, cbopaque,
                           "tcache:                  %12zu\n", tcache_bytes);
    ...
    if (json) {
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\t\"resident\": %zu%s\n",
                       resident, (bins || large) ? "," : "");
    } else {
        malloc_cprintf(write_cb, cbopaque, "resident:                %12zu\n",
                       resident);
    }
...
static void stats_general_print(void (*write_cb)(void *, const char *),
                                void *cbopaque, bool json, bool more) {
    const char *cpv;
    bool bv;
    unsigned uv;
    uint32_t u32v;
...
#define CONFIG_WRITE_BOOL_JSON(n, c)                                           \
    if (json) {                                                                \
        ...
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\"" #n "\": %s%s\n",         \
                       bv ? "true" : "false", (c));                            \
    }

    if (json) {
        malloc_cprintf(write_cb, cbopaque, "\t\t\"config\": {\n");
    }
    ...
    if (json) {
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\"debug\": %s,\n",
                       bv ? "true" : "false");
    } else {
    ...
    if (json) {
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\"malloc_conf\": \"%s\",\n",
                       config_malloc_conf);
    } else {
        malloc_cprintf(write_cb, cbopaque, "config.malloc_conf: \"%s\"\n",
                       config_malloc_conf);
    }
...
#define OPT_WRITE_UNSIGNED(n, c)                                               \
    if (je_mallctl("opt." #n, (void *)&uv, &usz, NULL, 0) == 0) {              \
        if (json) {                                                            \
            malloc_cprintf(write_cb, cbopaque, "\t\t\t\"" #n "\": %u%s\n", uv, \
                           (c));                                               \
        } else {                                                               \
            malloc_cprintf(write_cb, cbopaque, "  opt." #n ": %u\n", uv);      \
        }                                                                      \
    }
#define OPT_WRITE_SSIZE_T(n, c)                                                \
    if (je_mallctl("opt." #n, (void *)&ssv, &sssz, NULL, 0) == 0) {            \
        if (json) {                                                            \
            malloc_cprintf(write_cb, cbopaque, "\t\t\t\"" #n "\": %zd%s\n",    \
                           ssv, (c));                                          \
        } else {                                                               \
...
#define OPT_WRITE_CHAR_P(n, c)                                                 \
    ...
    if (json) {
        malloc_cprintf(write_cb, cbopaque, "\t\t\"opt\": {\n");
    } else {
...
#undef OPT_WRITE_CHAR_P
    ...
    /* arenas. */
    if (json) {
        malloc_cprintf(write_cb, cbopaque, "\t\t\"arenas\": {\n");
    }

    CTL_GET("arenas.narenas", &uv, unsigned);
    if (json) {
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\"narenas\": %u,\n", uv);
    } else {
    ...
    if (json) {
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\"quantum\": %zu,\n", sv);
    } else {
    ...
    if (json) {
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\"page\": %zu,\n", sv);
    } else {
    ...
    if (je_mallctl("arenas.tcache_max", (void *)&sv, &ssz, NULL, 0) == 0) {
        if (json) {
            malloc_cprintf(write_cb, cbopaque, "\t\t\t\"tcache_max\": %zu,\n",
                           sv);
        } else {
            malloc_cprintf(write_cb, cbopaque,
    ...
    if (json) {
        ...
        CTL_GET("arenas.nbins", &nbins, unsigned);
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\"nbins\": %u,\n", nbins);

        CTL_GET("arenas.nhbins", &uv, unsigned);
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\"nhbins\": %u,\n", uv);

        malloc_cprintf(write_cb, cbopaque, "\t\t\t\"bin\": [\n");
        for (i = 0; i < nbins; i++) {
            malloc_cprintf(write_cb, cbopaque, "\t\t\t\t{\n");

            CTL_M2_GET("arenas.bin.0.size", i, &sv, size_t);
            malloc_cprintf(write_cb, cbopaque, "\t\t\t\t\t\"size\": %zu,\n",
                           sv);
            ...
            malloc_cprintf(write_cb, cbopaque, "\t\t\t\t\t\"slab_size\": %zu\n",
                           sv);
        ...
        CTL_GET("arenas.nlextents", &nlextents, unsigned);
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\"nlextents\": %u,\n",
                       nlextents);

        malloc_cprintf(write_cb, cbopaque, "\t\t\t\"lextent\": [\n");
    ...
    /* prof. */
    if (config_prof && json) {
        malloc_cprintf(write_cb, cbopaque, "\t\t\"prof\": {\n");
        ...
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\"active\": %s,\n",
                       bv ? "true" : "false");

        CTL_GET("prof.gdump", &bv, bool);
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\"gdump\": %s,\n",
                       bv ? "true" : "false");

        CTL_GET("prof.interval", &u64v, uint64_t);
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\"interval\": %" FMTu64 ",\n",
                       u64v);

        CTL_GET("prof.lg_sample", &ssv, ssize_t);
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\"lg_sample\": %zd\n", ssv);

        malloc_cprintf(write_cb, cbopaque, "\t\t}%s\n", more ? "," : "");
    }
}

static void stats_print_helper(void (*write_cb)(void *, const char *),
                               void *cbopaque, bool json, bool merged,
                               bool destroyed, bool unmerged, bool bins,
                               bool large) {
    ...
    if (json) {
        malloc_cprintf(write_cb, cbopaque, "\t\t\"stats\": {\n");

        malloc_cprintf(write_cb, cbopaque, "\t\t\t\"allocated\": %zu,\n",
                       allocated);
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\"active\": %zu,\n", active);
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\"metadata\": %zu,\n",
                       metadata);
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\"resident\": %zu,\n",
                       resident);
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\"mapped\": %zu,\n", mapped);
        malloc_cprintf(write_cb, cbopaque, "\t\t\t\"retained\": %zu\n",
                       retained);
    ...
    if (merged || destroyed || unmerged) {
        ...
        if (json) {
            malloc_cprintf(write_cb, cbopaque, "\t\t\"stats.arenas\": {\n");
        }
        ...
        CTL_GET("arenas.narenas", &narenas, unsigned);
        {
            ...
            if (merged && (ninitialized > 1 || !unmerged)) {
                ...
                if (json) {
                    malloc_cprintf(write_cb, cbopaque, "\t\t\t\"merged\": {\n");
                } else {
                    malloc_cprintf(write_cb, cbopaque,
                                   "\nMerged arenas stats:\n");
                }
                stats_arena_print(write_cb, cbopaque, json, MALLCTL_ARENAS_ALL,
                                  bins, large);
                if (json) {
                    malloc_cprintf(write_cb, cbopaque, "\t\t\t}%s\n",
                                   (ninitialized > 1) ? "," : "");
            ...
            for (i = j = 0; i < narenas; i++) {
                if (initialized[i]) {
                    ...
                    stats_arena_print(write_cb, cbopaque, json, i, bins, large);
                    if (json) {
                        malloc_cprintf(write_cb, cbopaque, "\t\t\t}%s\n",
                                       (j < ninitialized) ? "," : "");
                    }
