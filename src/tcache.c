...
void *tcache_alloc_small_hard(tsdn_t *tsdn, arena_t *arena, tcache_t *tcache,
    ...
    arena_tcache_fill_small(tsdn, arena, tbin, binind,
    ...
    for (nflush = tbin->ncached - rem; nflush > 0; nflush = ndeferred) {
        ...
        for (i = 0; i < nflush; i++) {
            ...
            if (extent_arena_get(extent) == bin_arena) {
                arena_dalloc_bin_junked_locked(tsd_tsdn(tsd), bin_arena, extent,
                                               ptr);
            } else {
                /*
...
void tcache_bin_flush_large(tsd_t *tsd, tcache_bin_t *tbin, szind_t binind,
    ...
    for (nflush = tbin->ncached - rem; nflush > 0; nflush = ndeferred) {
        ...
        if ((config_prof || config_stats) && locked_arena == arena) {
            if (config_prof) {
                idump = arena_prof_accum_locked(arena, tcache->prof_accumbytes);
                tcache->prof_accumbytes = 0;
            }
            if (config_stats) {
                merged_stats = true;
    ...
    if (config_stats && !merged_stats) {
        ...
        arena->stats.nrequests_large += tbin->tstats.nrequests;
        arena->stats.lstats[binind - NBINS].nrequests += tbin->tstats.nrequests;
        tbin->tstats.nrequests = 0;
...
static void tcache_arena_associate(tsdn_t *tsdn, tcache_t *tcache,
                                   arena_t *arena) {
    if (config_stats) {
...
void tcache_arena_reassociate(tsdn_t *tsdn, tcache_t *tcache, arena_t *oldarena,
                              arena_t *newarena) {
...
static void tcache_destroy(tsd_t *tsd, tcache_t *tcache) {
    arena_t *arena;
    unsigned i;
    ...
    idalloctm(tsd_tsdn(tsd), iealloc(tsd_tsdn(tsd), tcache), tcache, NULL, true,
              true);
}
...
static void tcaches_elm_flush(tsd_t *tsd, tcaches_t *elm) {
    if (elm->tcache == NULL) {
        return;
    }
...
bool tcache_boot(tsdn_t *tsdn) {
    ...
    if (opt_lg_tcache_max < 0 ||
        (ZU(1) << opt_lg_tcache_max) < SMALL_MAXCLASS) {
    ...
    tcache_bin_info = (tcache_bin_info_t *)base_alloc(
        tsdn, b0get(), nhbins * sizeof(tcache_bin_info_t), CACHELINE);
    ...
    for (i = 0; i < NBINS; i++) {
        ...
        } else if ((arena_bin_info[i].nregs << 1) <= TCACHE_NSLOTS_SMALL_MAX) {
