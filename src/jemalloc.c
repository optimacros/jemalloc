...
static void a0idalloc(extent_t *extent, void *ptr, bool is_internal) {
...
#define CONF_HANDLE_T_U(t, o, n, min, max, check_min, check_max, clip)         \
    if (CONF_MATCH(n)) {                                                       \
        ...
        if (get_errno() != 0 || (uintptr_t)end - (uintptr_t)v != vlen) {       \
            ...
            if (CONF_MIN_##check_min(um, (min)))                               \
                o = (t)(min);                                                  \
            else if (CONF_MAX_##check_max(um, (max)))                          \
                o = (t)(max);                                                  \
            else                                                               \
                o = (t)um;                                                     \
        } else {                                                               \
            if (CONF_MIN_##check_min(um, (min)) ||                             \
                CONF_MAX_##check_max(um, (max))) {                             \
...
#define CONF_HANDLE_CHAR_P(o, n, d)                                            \
    if (CONF_MATCH(n)) {                                                       \
        ...
        continue;                                                              \
                ...
                for (i = 0; i < dss_prec_limit; i++) {
                    if (strncmp(dss_prec_names[i], v, vlen) == 0) {
                        if (extent_dss_prec_set(i)) {
                            malloc_conf_error("Error setting dss", k, klen, v,
                                              vlen);
                        } else {
            ...
            CONF_HANDLE_UNSIGNED(opt_narenas, "narenas", 1, UINT_MAX, yes, no,
                                 false)
            ...
            if (config_fill) {
                if (CONF_MATCH("junk")) {
                    if (CONF_MATCH_VALUE("true")) {
                        ...
                        malloc_conf_error("Invalid conf value", k, klen, v,
                                          vlen);
                    }
                    continue;
                }
            ...
            if (config_prof) {
                ...
                CONF_HANDLE_SIZE_T(opt_lg_prof_sample, "lg_prof_sample", 0,
                                   (sizeof(uint64_t) << 3) - 1, no, yes, true)
...
static bool malloc_init_hard_finish(tsdn_t *tsdn) {
    ...
    arenas = (arena_t **)base_alloc(
        tsdn, a0->base, sizeof(arena_t *) * (MALLOCX_ARENA_MAX + 1), CACHELINE);
    if (arenas == NULL) {
        return (true);
    }
...
static void *ialloc_prof_sample(tsd_t *tsd, size_t usize, szind_t ind,
                                bool zero, prof_tctx_t *tctx, bool slow_path) {
    void *p;
    ...
    if (usize <= SMALL_MAXCLASS) {
        ...
        arena_prof_promote(tsd_tsdn(tsd), iealloc(tsd_tsdn(tsd), p), p, usize);
    } else {
 ...
 */
JEMALLOC_ALWAYS_INLINE_C void *ialloc_body(size_t size, bool zero,
                                           tsdn_t **tsdn, size_t *usize,
                                           bool slow_path) {
    tsd_t *tsd;
    szind_t ind;
...
JEMALLOC_ALWAYS_INLINE_C void ialloc_post_check(void *ret, tsdn_t *tsdn,
                                                size_t usize, const char *func,
                                                bool update_errno,
                                                bool slow_path) {
...
static void *imemalign_prof_sample(tsd_t *tsd, size_t alignment, size_t usize,
                                   prof_tctx_t *tctx) {
    void *p;
...
JEMALLOC_ALWAYS_INLINE_C void *imemalign_prof(tsd_t *tsd, size_t alignment,
                                              size_t usize) {
    void *p;
    prof_tctx_t *tctx;
...
label_return:
    if (config_stats && likely(result != NULL)) {
        assert(usize ==
               isalloc(tsd_tsdn(tsd), iealloc(tsd_tsdn(tsd), result), result));
...
JEMALLOC_EXPORT JEMALLOC_ALLOCATOR JEMALLOC_RESTRICT_RETURN void
    ...
    if (unlikely(num_size == 0)) {
         ...
         */
                   ...
                   ((num | size) & (SIZE_T_MAX << (sizeof(size_t) << 2))) &&
                   (num_size / size != num))) {
...
static void *irealloc_prof_sample(tsd_t *tsd, extent_t *extent, void *old_ptr,
    ...
    if (usize <= SMALL_MAXCLASS) {
        p = iralloc(tsd, extent, old_ptr, old_usize, LARGE_MINCLASS, 0, false);
        if (p == NULL) {
            return (NULL);
        }
        arena_prof_promote(tsd_tsdn(tsd), iealloc(tsd_tsdn(tsd), p), p, usize);
...
JEMALLOC_ALWAYS_INLINE_C void *irealloc_prof(tsd_t *tsd, extent_t *old_extent,
                                             void *old_ptr, size_t old_usize,
                                             size_t usize) {
    void *p;
    extent_t *extent;
    bool prof_active;
    ...
    if (unlikely((uintptr_t)tctx != (uintptr_t)1U)) {
        p = irealloc_prof_sample(tsd, old_extent, old_ptr, old_usize, usize,
                                 tctx);
    } else {
        p = iralloc(tsd, old_extent, old_ptr, old_usize, usize, 0, false);
    }
    if (unlikely(p == NULL)) {
        prof_alloc_rollback(tsd, tctx, true);
        return (NULL);
...
JEMALLOC_INLINE_C void ifree(tsd_t *tsd, void *ptr, tcache_t *tcache,
                             bool slow_path) {
    extent_t *extent;
    size_t usize;
...
JEMALLOC_INLINE_C void isfree(tsd_t *tsd, extent_t *extent, void *ptr,
                              size_t usize, tcache_t *tcache, bool slow_path) {
    witness_assert_lockless(tsd_tsdn(tsd));
    ...
    if (likely(ptr != NULL)) {
        ...
        if (config_prof && opt_prof) {
            ...
            ret = unlikely(usize == 0 || usize > LARGE_MAXCLASS)
                      ? NULL
                      : irealloc_prof(tsd, extent, ptr, old_usize, usize);
        } else {
            if (config_stats) {
                usize = s2u(size);
            }
            ret = iralloc(tsd, extent, ptr, old_usize, size, 0, false);
        }
        tsdn = tsd_tsdn(tsd);
    } else {
...
JEMALLOC_ALWAYS_INLINE_C void *imallocx_flags(tsdn_t *tsdn, size_t usize,
...
static void *imallocx_prof_sample(tsdn_t *tsdn, size_t usize, size_t alignment,
    ...
    if (usize <= SMALL_MAXCLASS) {
        ...
        p = imallocx_flags(tsdn, LARGE_MINCLASS, alignment, zero, tcache, arena,
                           slow_path);
        if (p == NULL) {
            return (NULL);
        }
...
JEMALLOC_ALWAYS_INLINE_C void *imallocx_prof(tsd_t *tsd, size_t size, int flags,
    ...
    if (likely((uintptr_t)tctx == (uintptr_t)1U)) {
        ...
        p = imallocx_prof_sample(tsd_tsdn(tsd), *usize, alignment, zero, tcache,
                                 arena, slow_path);
    } else {
        p = NULL;
    }
...
JEMALLOC_ALWAYS_INLINE_C void *imallocx_no_prof(tsd_t *tsd, size_t size,
                                                int flags, size_t *usize,
                                                bool slow_path) {
    void *p;
    size_t alignment;
    bool zero;
    ...
    p = imallocx_flags(tsd_tsdn(tsd), *usize, alignment, zero, tcache, arena,
                       slow_path);
    assert(alignment == 0 || ((uintptr_t)p & (alignment - 1)) == ZU(0));
    return (p);
}
...
/* This function guarantees that *tsdn is non-NULL on success. */
JEMALLOC_ALWAYS_INLINE_C void *imallocx_body(size_t size, int flags,
                                             tsdn_t **tsdn, size_t *usize,
                                             bool slow_path) {
    tsd_t *tsd;
...
static void *irallocx_prof_sample(tsdn_t *tsdn, extent_t *extent, void *old_ptr,
                                  size_t old_usize, size_t usize,
                                  size_t alignment, bool zero, tcache_t *tcache,
    ...
    if (usize <= SMALL_MAXCLASS) {
        ...
        p = iralloct(tsdn, extent, old_ptr, old_usize, usize, alignment, zero,
                     tcache, arena);
    }
...
JEMALLOC_ALWAYS_INLINE_C void *irallocx_prof(tsd_t *tsd, extent_t *old_extent,
                                             void *old_ptr, size_t old_usize,
    ...
    if (unlikely((uintptr_t)tctx != (uintptr_t)1U)) {
        p = irallocx_prof_sample(tsd_tsdn(tsd), old_extent, old_ptr, old_usize,
                                 *usize, alignment, zero, tcache, arena, tctx);
    } else {
        p = iralloct(tsd_tsdn(tsd), old_extent, old_ptr, old_usize, size,
                     alignment, zero, tcache, arena);
    }
    if (unlikely(p == NULL)) {
    ...
    prof_realloc(tsd, extent, p, *usize, tctx, prof_active, false, old_extent,
                 old_ptr, old_usize, old_tctx);
...
JEMALLOC_EXPORT JEMALLOC_ALLOCATOR JEMALLOC_RESTRICT_RETURN void
    ...
    if (config_prof && opt_prof) {
        ...
        p = irallocx_prof(tsd, extent, ptr, old_usize, size, alignment, &usize,
                          zero, tcache, arena);
        if (unlikely(p == NULL)) {
            goto label_oom;
        }
    } else {
        p = iralloct(tsd_tsdn(tsd), extent, ptr, old_usize, size, alignment,
                     zero, tcache, arena);
        if (unlikely(p == NULL)) {
            goto label_oom;
        }
        if (config_stats) {
            usize = isalloc(tsd_tsdn(tsd), iealloc(tsd_tsdn(tsd), p), p);
        }
    }
    assert(alignment == 0 || ((uintptr_t)p & (alignment - 1)) == ZU(0));
...
JEMALLOC_ALWAYS_INLINE_C size_t ixallocx_helper(tsdn_t *tsdn, extent_t *extent,
                                                void *ptr, size_t old_usize,
...
static size_t ixallocx_prof_sample(tsdn_t *tsdn, extent_t *extent, void *ptr,
                                   size_t old_usize, size_t size, size_t extra,
                                   size_t alignment, bool zero,
                                   prof_tctx_t *tctx) {
    size_t usize;
...
JEMALLOC_ALWAYS_INLINE_C size_t ixallocx_prof(tsd_t *tsd, extent_t *extent,
                                              void *ptr, size_t old_usize,
                                              size_t size, size_t extra,
                                              size_t alignment, bool zero) {
    ...
    if (unlikely((uintptr_t)tctx != (uintptr_t)1U)) {
        usize = ixallocx_prof_sample(tsd_tsdn(tsd), extent, ptr, old_usize,
                                     size, extra, alignment, zero, tctx);
    } else {
        usize = ixallocx_helper(tsd_tsdn(tsd), extent, ptr, old_usize, size,
    ...
    prof_realloc(tsd, extent, ptr, usize, tctx, prof_active, false, extent, ptr,
                 old_usize, old_tctx);
